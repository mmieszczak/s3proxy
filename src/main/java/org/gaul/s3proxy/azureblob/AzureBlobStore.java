...
public final class AzureBlobStore extends BaseBlobStore {
    ...
    public MultipartPart uploadMultipartPart(MultipartUpload mpu,
        ...
        try (var is = payload.openStream();
            ...
            // BinaryData.fromStream streams in 8 KB chunks via InputStreamContent.
            // The stageBlock(String, InputStream, long) overload buffers 4 MB chunks
            // via Utility.convertStreamToByteBuffer — do not use it.
        ...
        } catch (UnexpectedLengthException ule) {
            throw new RuntimeException(
                    "Failed to upload part %d for blob '%s' in container '%s': %s".formatted(
                    partNumber, mpu.blobName(), mpu.containerName(), ule.getMessage()), ule);
        } catch (IOException ioe) {
            throw new RuntimeException(
                    "Failed to upload part %d for blob '%s' in container '%s': %s".formatted(
